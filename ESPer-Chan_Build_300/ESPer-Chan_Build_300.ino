...
void setupUSBSerial() {
#if USE_USB_CDC
    USB.begin(); USBSerial.begin(); delay(1000);
#endif
}
...
String sendAudioToWhisper(int16_t* audioBuffer, int sampleCount) {
    if (!wifiConnected) return "[ERROR: WiFi未接続]";
...
void speakWithVoicevox(const String& text) {
    ...
    while (stream->available() > 0 || stream->connected()) {
        ...
        if (rd > 0) {
            ...
            for (int i = 0; i < sc && fadeIn < FADE_IN_DUR; i++) {
                float f = (float)fadeIn / FADE_IN_DUR;
                samples[i] = (int16_t)(samples[i] * f); fadeIn++;
            }
#if USE_AVATAR
...
void startVoiceConversationFeedback() {
    ...
    setGifFaceIndex(9);
#endif
    // PipPip音は約200msで終わる。その後すぐ録音できるよう最小待機だけ残す
    for (int i = 0; i < 4; i++) { delay(50); webServer.handleClient(); }
}
...
void wakeWordTask(void *pvParameters) {
    ...
    while (true) {
        ...
        if (idx >= wakeSc && voiceState == VOICE_IDLE && detectWakeWordNeeNee(&buf[idx - wakeSc], wakeSc)) {
            // Stage2: Whisper STT で発話内容を確認し誤検知を排除する
            String stt = sendAudioToWhisper(&buf[idx - wakeSc], wakeSc);
            bool confirmed = (stt.indexOf("ねね") >= 0 ||
                              stt.indexOf("ねぇねぇ") >= 0 ||
                              stt.indexOf("ねえねえ") >= 0 ||
                              stt.indexOf("ね、ね、") >= 0);
            if (confirmed) {
...
void printBootScreen() {
    ...
    USER_SERIAL.println(F("========================================"));
#if MIC_TYPE == MIC_TYPE_I2S
    USER_SERIAL.println(F("[HW] Mic:INMP441 I2S(WS=15,SCK=16,SD=17) Speaker:MAX98357A I2S(GPIO1/2/3)"));
#else
    USER_SERIAL.println(F("[HW] Mic:PDM(GPIO41/42) Speaker:MAX98357A I2S(GPIO1/2/3)"));
#endif
    USER_SERIAL.println(F("[SW] LLM:lfm2.5-1.2b-instruct STT:whisper.cpp TTS:VOICEVOX"));
    USER_SERIAL.println(F("========================================"));
...
void setup() {
    ...
    if (appSettings.faceType == 1) {
        ...
        float customScale = (appSettings.displayType == DISPLAY_ST7789_240X320)  ? 1.00f :
                            (appSettings.displayType == DISPLAY_ST7735S_128X128) ? 0.40f : 0.30f;
        avatar.setScale(customScale);
        ...
        if (appSettings.displayType == DISPLAY_SSD1306) {
            ...
            float tftScale = (appSettings.displayType == DISPLAY_ST7789_240X320) ? 1.00f : 0.40f;
            avatar.setScale(tftScale);
            {
                ColorPalette cp;
                cp.set(COLOR_PRIMARY,    (uint16_t)0xFFFF);
                cp.set(COLOR_BACKGROUND, (uint16_t)0x0000);
