...
void startMusicMode() {
    ...
    if (appSettings.displayType != DISPLAY_SSD1306) {
        avatar.suspend();
        // DMA転送はタスクサスペンドでは止まらないため、完了まで待機
        delay(300);
    }
    #endif
    ...
    loadIndex();
    if (indexList.empty()) {
        USER_SERIAL.println(F("[INDEX] /index.csv が見つかりません。自動生成を開始します..."));
        generateIndex();
        if (indexList.empty()) {
...
String readID3Tag(const String& filepath, const char* frameID) {
    ...
    if (!f) {
        LOG_E("[ID3] Open failed: %s", filepath.c_str());
        return "";
    }

    uint8_t header[10];
    if (f.read(header, 10) < 10) { f.close(); return ""; }

    if (header[0] != 'I' || header[1] != 'D' || header[2] != '3') {
        LOG_I("[ID3] No ID3 tag: %s", filepath.c_str());
        f.close();
        return "";
    }

    uint8_t id3Major = header[3]; // 3 = v2.3, 4 = v2.4
    ...
    uint8_t* tagData = (uint8_t*)malloc(readSize);
    ...
    size_t pos = 10;
    bool hasExtendedHeader = (flags & 0x40);
    if (hasExtendedHeader && actualRead >= 14) {
        uint32_t extSize;
        if (id3Major >= 4) {
            extSize = ((tagData[10] & 0x7F) << 21) |
                      ((tagData[11] & 0x7F) << 14) |
                      ((tagData[12] & 0x7F) << 7) |
                      (tagData[13] & 0x7F);
        } else {
            extSize = ((uint32_t)tagData[10] << 24) |
                      ((uint32_t)tagData[11] << 16) |
                      ((uint32_t)tagData[12] << 8) |
                      tagData[13];
        }
        pos += 4 + extSize;
    }
    ...
    while (pos + 10 <= actualRead && frameCount < 64) {
        ...
        uint32_t frameSize;
        if (id3Major >= 4) {
            frameSize = ((uint32_t)(tagData[pos+4] & 0x7F) << 21) |
                        ((uint32_t)(tagData[pos+5] & 0x7F) << 14) |
                        ((uint32_t)(tagData[pos+6] & 0x7F) << 7) |
                        (tagData[pos+7] & 0x7F);
        } else {
            frameSize = ((uint32_t)tagData[pos+4] << 24) |
                        ((uint32_t)tagData[pos+5] << 16) |
                        ((uint32_t)tagData[pos+6] << 8) |
                        tagData[pos+7];
        }
        uint16_t frameFlags = (tagData[pos+8] << 8) | tagData[pos+9];

        if (frameSize == 0 || frameSize > 100000 || pos + 10 + frameSize > actualRead) break;

        uint8_t* frameData = tagData + pos + 10;

        if (strcmp(frameName, frameID) == 0) {
            result = parseID3Text(frameData, frameSize);
            USER_SERIAL.printf("[ID3] >>> MATCH %s result=\"%s\"\n", frameID, result.c_str());
        }
    ...
    free(tagData);
    if (result.length() == 0) {
        LOG_E("[ID3] Frame %s NOT FOUND in %s (scanned %d frames)", frameID, filepath.c_str(), frameCount);
    }
    return result;
}
...
void loadIndex() {
    ...
    if (!idxFile) {
        LOG_E("[INDEX] /index.csv が見つかりません。/index を実行して生成してください");
        return;
    }

    LOG_I("[INDEX] /index.csv を読み込み中...");
    while (idxFile.available()) {
        String line = idxFile.readStringUntil('\n');
        line.trim();
        if (line.length() == 0 || line.startsWith("filepath")) continue;
    ...
    LOG_I("[INDEX] %u 曲を読み込みました。", indexList.size());
    if (indexList.size() > 0) {
...
void generateIndex() {
    LOG_I("[INDEX] SDカードから index.csv を生成中...");
    LOG_I("[INDEX] ID3タグを読み込み中（時間がかかる場合があります）...");
    indexList.clear();
    playOrder.clear();
    currentTrackIdx = -1;
    ...
    LOG_I("[INDEX] %u 個のオーディオファイルを見つけました。タグをスキャン中...", audioFiles.size());

    for (size_t i = 0; i < audioFiles.size(); i++) {
        String fullPath = audioFiles[i];

        String artistFromFolder = "Unknown";
        String albumFromFolder = "Unknown";

        int lastSlash = fullPath.lastIndexOf('/');
        String dirPath = (lastSlash > 0) ? fullPath.substring(0, lastSlash) : "/";

        if (dirPath != "/") {
            String pathWithoutRoot = dirPath.substring(1);
            int firstSlash = pathWithoutRoot.indexOf('/');
            if (firstSlash == -1) {
                artistFromFolder = pathWithoutRoot;
            } else {
                artistFromFolder = pathWithoutRoot.substring(0, firstSlash);
                String remaining = pathWithoutRoot.substring(firstSlash + 1);
                remaining.replace("/", "");
                if (remaining.length() > 0) {
                    albumFromFolder = remaining;
                }
            }
        }
        ...
        if (fullPath.endsWith(".mp3") || fullPath.endsWith(".MP3")) {
            artist = readID3Tag(fullPath, "TPE1");
            album = readID3Tag(fullPath, "TALB");
            title = readID3Tag(fullPath, "TIT2");
            ...
            M4ATags m4aTags = readM4ATags(fullPath);
            artist = m4aTags.artist;
            album = m4aTags.album;
            title = m4aTags.title;
        }

        if (artist.length() == 0) artist = artistFromFolder;
        if (album.length() == 0) album = albumFromFolder;
        if (title.length() == 0) {
            title = getFilenameFromPath(fullPath);
            int dot = title.lastIndexOf('.');
            if (dot > 0) title = title.substring(0, dot);
            USER_SERIAL.printf("[INDEX] Fallback title from filename: \"%s\"\n", title.c_str());
        }

        USER_SERIAL.printf("[INDEX] %s -> title=\"%s\" artist=\"%s\" album=\"%s\"\n",
                      getFilenameFromPath(fullPath).c_str(),
        ...
        if ((i + 1) % 10 == 0 || i == audioFiles.size() - 1) {
            LOG_I("[INDEX] %u/%u をスキャン完了...", i + 1, audioFiles.size());
        }
    }
    ...
    for (size_t i = 0; i < indexList.size(); i++) {
        idxFile.printf("%s,%s,%s,%s\n",
                       csvEscape(indexList[i].filepath).c_str(),
                       csvEscape(indexList[i].album).c_str(),
                       csvEscape(indexList[i].artist).c_str(),
                       csvEscape(indexList[i].title).c_str());
    }
    idxFile.close();

    LOG_I("[INDEX] /index.csv を生成しました（%u 曲）。", indexList.size());
...
void playTrack(int orderIdx) {
    ...
    String displayName = getFilenameFromPath(currentFile);
    LOG_I("[PLAY] [%d/%u] %s", orderIdx + 1, playOrder.size(), displayName.c_str());
    LOG_I("[PLAY] アーティスト: %s | アルバム: %s", indexList[indexPos].artist.c_str(), indexList[indexPos].album.c_str());

    audio.stopSong();
    eofTriggered = true;  // stopSong直後のaudio.loop()でaudio_eof_mp3が発火しても再帰しないようにブロック
    for (int i = 0; i < 20; i++) {
        audio.loop();
        delay(10);
    }
...
void printPlaylist() {
    ...
    for (size_t i = 0; i < indexList.size(); i++) {
        ...
        LOG_I("[PLAYLIST] %s %2d: %s | %s | %s", marker.c_str(), i, indexList[i].artist.c_str(), indexList[i].album.c_str(), displayName.c_str());
    }
    USER_SERIAL.println(F("========================================"));
}
...
M4ATags readM4ATags(const String& filepath) {
    ...
    uint32_t ilstEnd = 0;
    const uint32_t SEARCH_LIMIT = 2 * 1024 * 1024; // 先頭2MBを検索

    // 先頭から ilst box を検索
    while (f.position() < SEARCH_LIMIT && f.position() < f.size()) {
        if (f.read(buf, 8) != 8) break;
        uint32_t size = mp4BoxSize(buf);
        char type[5];
        mp4BoxType(buf + 4, type);

        if (strcmp(type, "ilst") == 0) {
            foundIlst = true;
            ilstEnd = f.position() + size - 8;
            break;
        }

        if (size == 0) break;
        if (size == 1) {
            // extended size (64bit) - skip
            uint8_t ext[8];
            f.read(ext, 8);
            if (!f.seek(f.position() + size - 16)) break;
        } else if (size > 8) {
            if (!f.seek(f.position() + size - 8)) break;
        }
    }
    ...
    while (f.position() < ilstEnd) {
        if (f.read(buf, 8) != 8) break;
        uint32_t itemSize = mp4BoxSize(buf);
        char itemType[5];
        mp4BoxType(buf + 4, itemType);

        if (itemSize == 0 || itemSize > (ilstEnd - f.position() + 8)) break;

        uint32_t itemEnd = f.position() + itemSize - 8;
        String value = "";

        // item box 内をスキャンして data box を探す
        while (f.position() < itemEnd) {
            if (f.read(buf, 8) != 8) break;
            ...
            if (strcmp(childType, "data") == 0 && childSize >= 16) {
                ...
                uint32_t remaining = childSize - 16 - (textLen > 512 ? 512 : textLen);
                if (remaining > 0 && remaining < itemSize) {
                    f.seek(f.position() + remaining);
                }
            } else if (childSize > 8 && childSize < itemSize) {
                if (!f.seek(f.position() + childSize - 8)) break;
            } else {
                break;
            }
        }
        ...
        // itemEnd まで seek
        if (f.position() < itemEnd && itemEnd <= ilstEnd) {
            f.seek(itemEnd);
        }
    }

    f.close();
    USER_SERIAL.printf("[M4A] %s -> title=\"%s\" artist=\"%s\" album=\"%s\"\n",
                  getFilenameFromPath(filepath).c_str(),
                  tags.title.c_str(), tags.artist.c_str(), tags.album.c_str());
    return tags;
}
