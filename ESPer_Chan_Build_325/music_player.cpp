...
// 未対応M4A(libfaad未実装機能)のデコードエラー回数。1曲ごとにplayTrack()でリセット。
...
void onAudioEvent(Audio::msg_t m) {
    ...
    } else if (m.e == Audio::evt_log) {
        // 一部のM4A(AAC Main Profile / Channel Coupling等)はlibfaadが未対応で、
        // 再生開始直後に大量のデコードエラーが出て無音/ノイズになる。
        // 該当エラーが閾値を超えたら自動的に次の曲へスキップする。
        // カウンタはファイルスコープ（playTrack()で1曲ごとにリセットされる）。
        if (m.msg && (strstr(m.msg, "not yet implemented") ||
                      strstr(m.msg, "not allowed by specification") ||
                      strstr(m.msg, "Invalid number of channels"))) {
            decodeErrCount++;
            if (decodeErrCount > 5 && !eofTriggered) {
                USER_SERIAL.printf("[SKIP] decode errors (%d) - skipping unsupported track\n",
                                   decodeErrCount);
                USER_SERIAL.flush();
                eofTriggered = true;   // 二重スキップ防止（evt_eof等との競合をガード）
                playNext();            // 次の曲もエラーならまた次へ進む（それはOK）
            }
        }
    }
}
...
void audio_process_i2s(int32_t* outBuff, int16_t validSamples, bool* /*continueI2S*/) {
    ...
    // validSamples は int16_t。リサンプラ経由（ソースSR≠I2S SR）でアップサンプルされた曲は
    // フレーム数が増え、稀に int16_t の上限(32767)を超えて負値に化ける。負値ループは
    // 1回も回らず音はそのまま素通りするだけだが、念のためガードして範囲外アクセスも防ぐ。
    if (validSamples <= 0) return;

    for (int i = 0; i < validSamples; i++) {
        // 「int16 << 16」形式 → float [-1.0, 1.0] に変換（符号付き算術右シフト）
        // L/R両方を読んで平均化する。片チャンネルだけ読むと、左右に偏った
        // ステレオ音源（右寄りパンニング等）で無音になるバグがあった。
        float inL = (float)(outBuff[i * 2]     >> 16) * (1.0f / 32768.0f);
        float inR = (float)(outBuff[i * 2 + 1] >> 16) * (1.0f / 32768.0f);
        ...
        outBuff[i * 2]     = s;
        outBuff[i * 2 + 1] = s;  // L+R平均をモノラル合成して出力（リバーブ自体はステレオ感を持たせない設計）
    }
}
...
void playTrack(int orderIdx) {
    ...
    eofTriggered = false;
    decodeErrCount = 0;  // 未対応M4Aのデコードエラー自動スキップ用カウンタを曲ごとにリセット
    lastCur = 0;
    lastDur = 0;
