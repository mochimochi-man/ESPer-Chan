...
void loadSettings() {
    ...
    appSettings.musicAutoPlay = prefs.getInt("mautop",  DEFAULT_MUSIC_AUTO_PLAY);
    prefs.end();
}
...
void saveSettings() {
    ...
    prefs.putInt("mautop",     appSettings.musicAutoPlay);
...
String generateConfigPage() {
    ...
    html += "<label>ミュージック起動時自動再生</label><select name='mautop'>";
    html += String("<option value='0'") + (appSettings.musicAutoPlay == 0 ? " selected" : "") + ">しない</option>";
    html += String("<option value='1'") + (appSettings.musicAutoPlay == 1 ? " selected" : "") + ">曲リスト順</option>";
    html += String("<option value='2'") + (appSettings.musicAutoPlay == 2 ? " selected" : "") + ">ランダム</option>";
    html += String("<option value='3'") + (appSettings.musicAutoPlay == 3 ? " selected" : "") + ">アーティスト順</option>";
    html += "</select>";
...
void startAPMode() {
    ...
    webServer.on("/save", HTTP_POST, []() {
        ...
        if (webServer.hasArg("mautop"))    appSettings.musicAutoPlay  = webServer.arg("mautop").toInt();
