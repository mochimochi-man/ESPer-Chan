...
// ============================================
// SPI2 GPIO 再ルーティング + TFT パネル再初期化 (MusicMode終了時)
// ============================================
// startMusicMode() 内の SPI.begin(SD_SCK,...) が SPI2 GPIO を SD 用に切り替えるため、
// 復帰時に SPI.begin(TFT_SCK,...) で TFT 用に戻し、パネル初期化コマンドを再送する。
void reinitDisplaySPI() {
    ...
    // _tft_bus.release() は省略する。
    // release() は spi_bus_free() で GDMA チャンネルを解放するが、
    // avatar.init(16) 等でスプライトが多くPSRAMが逼迫している場合、
    // 直後の spi_bus_initialize() による DMA チャンネルの再確保が失敗し、
    // search_dma_out_ch() が -1 を返して DMA 転送レジスタが未設定のまま白画面になる。
    // release() を省略すると _spi_dev_handle が非 null のまま維持され、
    // spi::init() 内の spi_bus_initialize() がスキップされて既存の GDMA チャンネルを
    // そのまま再利用できる。SPI.begin(TFT_SCK,...) による GPIO 再ルーティングと
    // SPI2 レジスタ再設定・パネル初期化コマンド送信は initXXX() 経由で行われる。
